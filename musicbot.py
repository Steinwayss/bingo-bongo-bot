...
bot_intents = (
    discord.Intents.all()
)  # This gives the bot permission to (at least read messages and join voice) everything.
client = commands.Bot(command_prefix=config["prefix"], intents=bot_intents)

status = [
    "Macht was immer ein Bingo Bongo Bot so macht!",
    "Reinigt den Maschinenraum",
    "Spielt den selben Song nochmal",
    "Ist der König der Bongos",
    "Spielt Bingo",
...
async def play_from_queue(ctx: Context):
    ...
    async with ctx.typing():
        player = await YTDLSource.from_url(
            song["url"], loop=client.loop, stream=True
        )  # STREAM SET TRUE
        voice_channel.play(
...
"""get a stupid, random text sent via dm
As this is peak 2010s humor, it is removed for now"""
# @client.command(name='dm', help='surprise :)')
# async def dm(ctx):
#     await ctx.author.send(choice(dm_texts))


"""Change bot status every 60 seconds"""
...
async def control_authorized(ctx):
    """
    Join the voice channel of the messages author if possible.
    Returns
    """
